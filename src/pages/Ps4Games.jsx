...
export default function Ps4Games() {
  const Ps4Games = Games.filter((game) => {
    return game.type === "PS4"
  })
  
  const popularImgEl = Ps4Games.map((game) => {
    return (
      <div key={game.id}>
        ...
        <Link to={`../all-games/${game.id}`} dataset={game.title}>View Product</Link>
      </div>
    );
  });

  useEffect(() => {
    document.title = "PS4 Games"
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
